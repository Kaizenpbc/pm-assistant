...
const accuracyInsightsPrompt = new PromptTemplate(
  `You are an AI performance analyst for a project management system used by Guyana's Ministry of Works. Analyze the accuracy report below and provide actionable improvement suggestions.
...
export class AILearningServiceV2 {
  ...
  recordAccuracy(record: AIAccuracyRecord): void {
    ...
    db.query(
      `INSERT INTO ai_accuracy_tracking (id, project_id, task_id, metric_type, predicted_value, actual_value, variance_pct, project_type, region_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        record.projectId,
  ...
  async buildLearningContext(projectType?: string, regionId?: string): Promise<string> {
    const db = (this.fastify as any).mysql || (this.fastify as any).db;
    if (!db) return '';
    ...
    try {
      ...
      for (const r of rows) {
        const accuracy = Math.max(0, 100 - parseFloat(r.avg_var));
        context += `- ${r.metric_type}: ${accuracy.toFixed(1)}% accurate (${r.cnt} samples, avg variance ${parseFloat(r.avg_var).toFixed(1)}%)\n`;
      }
