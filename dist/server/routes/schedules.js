...
async function scheduleRoutes(fastify) {
    fastify.get('/project/:projectId', {
        ...
        try {
            ...
            const schedules = await connection_1.databaseService.query('SELECT * FROM project_schedules WHERE project_id = ? ORDER BY created_at DESC', [projectId]);
            return { schedules };
        }
        catch (error) {
    ...
    fastify.post('/', {
        ...
        try {
            ...
            const existingProject = await connection_1.databaseService.query('SELECT id FROM projects WHERE id = ?', [scheduleData.projectId]);
            if (existingProject.length === 0) {
                if (scheduleData.projectId === '3') {
                    await connection_1.databaseService.query(`INSERT INTO projects (id, code, name, description, status, priority, budget_allocated, budget_spent, currency, start_date, end_date, project_manager_id, created_by) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                        '3',
                        'DES-2024',
            ...
            await connection_1.databaseService.query(`INSERT INTO project_schedules (id, project_id, name, description, start_date, end_date, created_by) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                scheduleId,
            ...
            const schedule = await connection_1.databaseService.query('SELECT * FROM project_schedules WHERE id = ?', [scheduleId]);
    ...
    fastify.put('/:scheduleId', {
        ...
        try {
            ...
            await connection_1.databaseService.query(`UPDATE project_schedules SET ${updateFields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, updateValues);
            const schedule = await connection_1.databaseService.query('SELECT * FROM project_schedules WHERE id = ?', [scheduleId]);
    ...
    fastify.get('/:scheduleId/tasks', {
        ...
        try {
            const { scheduleId } = request.params;
            const tasks = await connection_1.databaseService.query('SELECT * FROM tasks WHERE schedule_id = ? ORDER BY created_at ASC', [scheduleId]);
            return { tasks };
        }
        catch (error) {
    ...
    fastify.post('/:scheduleId/tasks', {
        ...
        try {
            ...
            const createdBy = user?.userId || 'admin-001';
            await connection_1.databaseService.query(`INSERT INTO tasks (id, schedule_id, name, description, status, priority, assigned_to, due_date, created_by) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                taskId,
                scheduleId,
                taskData.name,
            ...
            const task = await connection_1.databaseService.query('SELECT * FROM tasks WHERE id = ?', [taskId]);
    ...
    fastify.put('/:scheduleId/tasks/:taskId', {
        ...
        try {
            ...
            await connection_1.databaseService.query(`UPDATE tasks SET ${updateFields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, updateValues);
            const task = await connection_1.databaseService.query('SELECT * FROM tasks WHERE id = ?', [taskId]);
            return { task: task[0] };
        }
        catch (error) {
    ...
    fastify.delete('/:scheduleId', {
        ...
        try {
            const { scheduleId } = request.params;
            await connection_1.databaseService.query('DELETE FROM tasks WHERE schedule_id = ?', [scheduleId]);
            await connection_1.databaseService.query('DELETE FROM project_schedules WHERE id = ?', [scheduleId]);
